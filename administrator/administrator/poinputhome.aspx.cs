...
{
    public partial class poinputhome : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                GridView1.DataSource = getdata("SELECT * from poinput order by num");
                GridView1.DataBind();
            }
        }
        ...
        {
            string part = "", num = "", supplier = "", qty = "";
            GridViewRow row = GridView1.SelectedRow;
            part = row.Cells[2].Text;
            string charge = part;
            SqlConnection conn2 = new SqlConnection(ConfigurationManager.ConnectionStrings["dbconnection"].ConnectionString);
            SqlCommand cmd1 = new SqlCommand("SELECT * from poinput where partno='" + charge + "'", conn2);
            SqlDataReader dbr;
            conn2.Open();
            dbr = cmd1.ExecuteReader();
            while (dbr.Read())
            {
                num = Convert.ToString(dbr["num"]);
                supplier = Convert.ToString(dbr["suppliername"]);
                part = Convert.ToString(dbr["partno"]);
