...
{
    public partial class unitofmeasurehome : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                GridView1.DataSource = getdata("SELECT unit,description FROM unitofmeasure order by unit");
                GridView1.DataBind();
            }
        }
        ...
        {
            string unitdemo = "",no="",u="",des="";
            GridViewRow row = GridView1.SelectedRow;
            unitdemo = row.Cells[0].Text;
            string unit = unitdemo;
            SqlConnection conn2 = new SqlConnection(ConfigurationManager.ConnectionStrings["dbconnection"].ConnectionString);
            SqlCommand cmd1=new SqlCommand("SELECT * from unitofmeasure where unit='"+unit+"'",conn2);
            SqlDataReader dbr;
            conn2.Open();
        ...
        {
            Response.Redirect("~/unitofmeasurehome.aspx", false);
        }
    }
}